...
`
     ...
     * 插件不能改 `req`，所以这一行挂在详情行下：`other · medium`（审核表命中的行 id · 等级）/
     * `NEEDS-HUMAN-TOKEN · 关键词转人工`（关键词带命中词）/ `判定失败转人工` / `内容超过送审上限`。
     *
     * **审核表命中时不再重复写「审核表转人工」**：框已经弹出来了，「转人工」这半句是重复信息；
     * `path`（criteria-human / keyword-human / …）是人**点开审批记录**时才看的东西，那里照旧显示。
     * 有 id 时这一行就是 id + 等级，没有 id 时才退回处置标签。
     * 审核模型真给过理由时（`humanVerdictReason`）再另起一行附上它自己那句理由。
     * 事件里没有可用判定就返回空串（那就不写这一行，宁可少一句也不瞎写）。
     *
     * `text` 是详情行里「操作」那一段（可能为 null），只用来决定理由给多长的额度：
     * 「操作」本身有 600 字的硬预算，它占满了，理由就写得更短。
     */
    function verdictLineFromEvent(t, ev, text) {
      const e = ev && typeof ev === 'object' ? ev : null
      if (!e) return ''
      const path = String(e.path || e.verdict || '')
      if (!path) return ''
      // 复核框（模型主动求的那次）没有「机器的判定」可言：它的标题已经写清是模型求的复核，
      // 再写一行「自动判定：模型请求人工复核」就是自指噪声。**但理由照写**：那是审核模型
      // 真的说过的一句话（判定没跑成时事件里本来就没有理由字段）。
      const labelLine = path === 'human-review' ? '' : (function () {
      ...
      const failed = JUDGE_FAILED_SRCS.indexOf(src) !== -1
      const label = failed ? (verdictLabel(t, 'judge-failed') || path) : (verdictLabel(t, path) || path)
      /**
       * 命中的**审核行 id**：`path` 只是「怎么处置」（criteria-* + human/reject/allow），
       * **是哪一行判的**只有 `judge.criterion` / `category` 里有。历史 tab 的审计区、决策事件、
       * 宿主侧复核框（`bulk · high`）都显示 id，审批框这一行同样要有（否则人只看到「审核表转人工」，
       * 不知道模型落的是哪一行）。
       * `src=none`（模型答了、类别认不出，程序落兜底行）**不写**：那个 id 是失败关闭的产物，
       * 写出来等于替模型的输出背书（它连类别都没给对）；审计区的「判定来源」显示「认不出」。
       */
      const row = failed ? '' : (e.judge && e.judge.criterion) || e.category || ''
      const criterion = src === 'none' ? '' : String(row)
      ...
      // 兜底档要标出来（与宿主复核框里的注记同源）：`medium` 是模型给的，还是等级认不出落下来的，
      // 对人不是同一件事。
      const fallback = String(e.levelSrc || (e.judge && e.judge.levelSrc) || '') === 'fallback'
        ? t('detail.levelFallback') : ''
      // 命中审核行：`<id> · <等级>`；没有 id（关键词 / 判定失败 / 超上限）才写处置标签，
      // 那时标签是唯一说得清「为什么弹框」的东西。
      const head = criterion ? criterion : label
      return t('approval.autoVerdict') + head
        + (criterion
          ? (level ? ' · ' + levelLabel(t, level) + fallback : '')
          : (level ? ' · ' + levelLabel(t, level) + fallback : (keyword ? '（' + keyword + '）' : '')))
      })()
      const reason = humanVerdictReason(t, e, text)
      // 两段都空就不写这一行；有理由没标签时**不能**留一个空行（那一段自带前导换行，
      // 是给「标签 + 换行 + 理由」那个形状用的）。
      if (!labelLine) return reason ? reason.replace(/^\n/, '') : ''
      return labelLine + reason
    }

    /** 跟 `verdictLineFromEvent` / 宿主 `JUDGE_FAILURE_SRCS` 同一份闭集（三处不能互相 import）。 */
    const JUDGE_FAILED_SRCS = ['empty', 'timeout', 'call', 'route', 'plugin']

    /**
     * 审批框里接在判决后面的「审核理由」那一段。**只写审核模型自己说过的理由。**
     *
     * 审核理由此前只在「审批」tab 展开后的审计区（`detail.judgeReason`）里能看，人在框前决定放不放行
     * 时看不到；而那一句本来就是审核模型分类的依据。
     *
     * 四类**不写**（宁可少一句，也不让人把别的东西读成模型的结论）：
     *   ① 判定压根没跑成（src ∈ JUDGE_FAILED_SRCS）——那一格装的是闭集证据（`err.judgeEffort` /
     *      `err.judgePayloadOversize request=N>B`），不是审核理由，判决行那句「判定失败转人工」
     *      已经说清结局，原始证据仍在「审批」tab；
     *   ② `judge.failed`（同一件事的另一处标记，宿主两条路都会打）；
     *   ③ `src=truncated`（判定路径上的送审超预算：判决行已写「内容超过送审上限」）；
     *   ④ 关键词人工桶那种**压根没问过模型**的转人工（事件里没有任何理由字段）。
     * 复核框（`path=human-review`）**有理由就写**：判决行被刻意留空（标题已说明是模型求的复核），
     * 但审核模型说过的那句话仍然是机器给过的结论；判定失败时宿主本来就不填理由字段，所以
     * 「失败不写」在这一路是自动成立的，不必为它加分支。
     */
    function humanVerdictReason(t, ev, text) {
      const e = ev && typeof ev === 'object' ? ev : null
      if (!e) return ''
      const src = String(e.src || '')
      if (JUDGE_FAILED_SRCS.indexOf(src) !== -1) return ''
      if (src === 'truncated') return ''
      const judge = e.judge && typeof e.judge === 'object' ? e.judge : null
      if (judge && judge.failed) return ''
      // `judge.reason` 优先：`judge` 才是判定现场的存档（`judgeReason` 是同一份的展示位）。
      const raw = judge && judge.reason ? judge.reason : e.judgeReason
      const one = String(raw == null ? '' : raw).replace(/\s+/g, ' ').trim()
      if (!one) return ''
      // 单行 + 定长：审批框的详情行是普通文本节点（换行会被折成空格），理由必须自己收口。
      // 「操作」那段占满了自己的 600 字预算时理由只留 80 字，别把要批准的内容挤下去。
      const cap = text && String(text).length > 600 ? 80 : 200
      const cut = one.slice(0, cap)
      const shown = one.length > cut.length ? cut + t('approval.judgeCut', { n: one.length }) : cut
      return '\n' + t('approval.judgeReason', { text: shown })
    }

    /**
     * `sessionId:callId:操作摘要长度` → 那一行（进程内缓存：审批框每次挂载只查一次；有界）。
     ...
     * 第三段是**操作摘要的长度**：理由的截断额度按它算，同一格在不同长度下是不同的一行。
     */
    const approvalVerdictCache = new Map()
    const verdictCacheKey = function (sessionId, callId, text) {
      return String(sessionId || '') + ':' + String(callId || '') + ':' + (text ? String(text).length : 0)
    }
    ...
    function ApprovalDetail(props) {
      ...
      // 它排在判决那一钩**前面**：判决那一行要按「操作」的长度决定理由的截断额度，这一钩的求值
      // 因此必须早于它下面的 `cacheKey` 与 useState 惰性初值。
      const text = (typeof useChat === 'function')
        ...
        : null
      // 缓存键除了会话与调用，还要带**操作摘要的长度**：理由那一段的截断额度是按它算的，
      // 同一格在别的长度下算出来的是另一行。
      const cacheKey = verdictCacheKey(frame.sessionId, callId, text)
      ...
      React.useEffect(function () {
        ...
        rpc('events', { sessionId: frame.sessionId, callId: callId }).then(function (data) {
          ...
          // 事件的最后一行就是这次调用的判决（`manual-pending` 带判定，见宿主 `forwardToHuman`）；
          // 审核理由（`humanVerdictReason`）接在同一行里，只写审核模型真的说过的理由。
          const line = verdictLineFromEvent(t, rows[rows.length - 1], text)
