...
2. 按需改**关键词**、**审核表**（每行三格：低/中/高 → 动作）、**风险等级**（三档说明 + 等级认不出时按哪一档）和**无法判定时**（缺工具参数 / 参数被截断：转人工或拒绝）。恢复默认按钮各带语言（审核表、等级说明、提示词各有中英两份），选中的语言同时决定提示词语言。
3. **审核模型** — 空则跟随部署默认。与提示词、超时一起保存。
4. **审核提示词** — 可改；恢复默认写回所选语言的出厂模板并把语言切过去。用 `{{criteria}}` 插入当前审核表。
5. 会话权限选 **自动审批**。

升级说明：审核表一行从「一个 action」变成「三格 action」，老文件的 `action` 会自动播种到三格，**动作与旧版逐字节一致**，只有你自己把三格拉开了才有区别；`action` 字段会在下次写盘时消失。`other` 不再是特殊行——不可删除，但**说明与三格都可改**，它现在还承接「输出认不出」「判定没跑成」。审核提示词模板加了 `{{levels}}` 与第三行输出；**自定义模板没要求输出等级时，等级一律按「等级认不出时按」那一档执行**（默认 high）。

## 怎么判定

仅当会话预设是「自动审批」时介入。`workspace-write` 下，工作区内写入不会进审批。`danger-full-access` 走同一条管道。

管道：**关键词 → 缺参/截断开关 → 审核模型（类别 + 等级）→ 按 (行, 等级) 查三格动作**。非表内结果（输出认不出、空输出、超时、调用失败、没有可用路由、插件异常）一律按**兜底行 `other` 的三格**执行；请求被取消不产生判定。**自定义工具（MCP 等）参数名不在已知字段里不算缺参**：参数连键名（含 `params.command` 这类嵌套路径）一起送审核模型，也进关键词干草。只有「工具真没给任何字符串参数」和「字段被截断」走「无法判定时」的两个开关（默认转人工，禁止自动放行）。

1. **关键词**（拒绝 > 人工 > 允许）。匹配工具名、命令、路径和工作目录，**以及自定义工具那些不认识名字的参数值**（否则 `{cmd: 'rm -rf /'}` 这类调用会绕过红线）；允许桶只认命令/路径/工作目录，参数名认不出时绝不自动放行。允许词不匹配工具名，也不匹配会话目录名。**出厂拒绝词只留三类「零上下文就确定灾难、不该让模型有发言权」的红线**：① 清根 `rm -rf /`（写法连 `rm -rf /*`、`sudo rm -rf /` 一起覆盖）；② 裸设备覆写与格式化（`of=/dev/`、`mkfs`、`wipefs`、`Format-Volume`、`Clear-Disk`、`diskutil eraseDisk`）；③ **门控自身的配置**（`.dsh/auto-approve`、`.dsh/profiles`、`.dsh/config.yml`、`cordis.patch.yml`、`auto-approve/allowlist`）与**私钥/云端凭据**（`id_rsa`/`id_ed25519`/`id_ecdsa`/`id_dsa`、`.pem`/`.p12`/`.pfx`/`.jks`、`authorized_keys`、`.netrc`、`.git-credentials`、`.pypirc`、`~/.aws/credentials`、`~/.kube/config`）。公钥不算（`id_rsa.pub`），dd 写 `/dev/null` 这类伪设备不算。
...
| other | 风险行和常规可回补都不符合，或拿不准；看着无害但确认不了也选它（兜底行：不能删除；还承接「输出认不出」「判定没跑成」） | 人工 |

风险等级默认三档（说明可改，等级 id 固定）：

| 等级 | 默认说明 | 出厂三格 |
|---|---|---|
| low | 可原样回补、影响范围限于本次工作区或临时产物 | 按各行出厂动作 |
| medium | 可回补但需要额外步骤，或只影响本机配置、缓存、工作区外文件 | 同上 |
| high | 不可回补，或影响远端、生产、他人、系统与凭据 | 同上 |
...
| 路径 | 用途 |
|---|---|
| `auto-approve/allowlist.json` | 关键词、审核表（三格动作）、风险等级说明与兜底档、缺参/截断开关、审核超时 |
