...
export const zh = {
  ...
  'set.confirm': '确认',
  'set.delete': '删除',
  'set.criteriaSub': '审核模型只看 id 和说明：说明写清「什么情况下选这个 id」。id 用英文（字母、数字、-、_），会显示在审批历史里；「其他」是兜底行——不能删除，说明也固定（只能改动作）。',
  'set.descPlaceholder': '说明（什么情况下选这个 id）',
  'set.criterionOtherNote': '兜底行：说明固定，只能改动作',
  ...
  'set.judgePromptHint': '用 {{criteria}} 插入当前审核表；删掉占位符时会把审核表附在末尾。与审核模型、超时一起保存。',
  ...
  'err.criterionOtherFixed': '「其他」是兜底行：说明固定，只能改动作',
...
export const en = {
  ...
  'set.confirm': 'Confirm',
  'set.delete': 'Delete',
  'set.criteriaSub': 'The judge sees only the id and description: state when to pick this id. Use an English id (letters, digits, -, _); it is shown in the approval history. "Other" is the fallback row: it cannot be deleted and its description is fixed (only the action can change).',
  'set.descPlaceholder': 'Description (when to pick this id)',
  'set.criterionOtherNote': 'Fallback row: description fixed, action editable',
  ...
  'set.judgePromptHint': 'Use {{criteria}} to insert the current criteria table; if the placeholder is removed, the table is appended. Saved with the judge model and timeout.',
  ...
  'err.criterionOtherFixed': '“Other” is the fallback row: its description is fixed, only the action can change',
